        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
               ...
               try {
                   ...
                   int progress = Integer.parseInt(cols[cols.length - 1].trim());

                   Task task = new Task(taskId, title, parentId, level, progress, status, startDate, endDate);
                   task.setAssignee(assignee);
                   task.setPriority(priority);
                   tasks.add(task);
               } catch (Exception ex) {
                   System.err.println("CSV import skipped invalid line " + lineNumber + ": " + ex.getMessage());
               }
            }
        }
        return tasks;
    ...
    private static void exportDelimited(File file, List<Task> tasks, String header, String delimiter, boolean useBom, boolean quoteFields) throws IOException {
        try (OutputStream os = new FileOutputStream(file);
            ...
            for (Task t : tasks) {
               ...
               values.add(normalizeForDelimitedField(t.getStatus(), quoteFields));
               values.add(normalizeForDelimitedField(t.getPriority(), quoteFields));
